...
export function LogoutModal({ open, onClose, onConfirm, userName = "Người dùng", userInitials = "U", userPlan = "Free" }: LogoutModalProps) {
  ...
  useLayoutEffect(() => { setMounted(true) }, []) // eslint-disable-line
  ...
  // createPortal lifts the modal out of the header's stacking context
  // (backdrop-filter on <header> creates a new stacking context that traps
  //  fixed-positioned children — portal to body fixes this)
  return createPortal(
    <AnimatePresence>
      {open && (
        <motion.div
            ...
            <div className="p-6">
              ...
              <div className="flex justify-center mb-5">
                ...
                >
                  ...
                  <div className="absolute -top-1 -right-1 w-6 h-6 rounded-full bg-orange-500/15 border border-orange-400/30 flex items-center justify-center">
              ...
              <div className="text-center mb-5">
                <h2 className="text-xl font-extrabold mb-2">Xác nhận đăng xuất</h2>
              ...
              <div className="flex items-center gap-3 p-3.5 rounded-xl bg-muted/50 border border-border/50 mb-6">
                <div className="w-10 h-10 rounded-full bg-linear-to-br from-blue-500 to-violet-600 flex items-center justify-center shadow-md shrink-0">
                  <span className="text-white font-extrabold text-sm">{userInitials}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold truncate">{userName}</p>
                  <p className="text-xs text-muted-foreground">Đang đăng nhập với gói {userPlan}</p>
                </div>
