...
import {
  Search, Sun, Moon, ShoppingCart, Bell,
  BookOpen, TrendingUp, User, LogOut, Settings,
  Menu, ChevronDown, Sparkles, X, Package,
  Lock, Heart, Bot,
} from "lucide-react"
...
const QUICK_TABS = [
  { icon: Bot, label: "AI Tools", href: "/ai-tools", color: "text-blue-400", hoverBg: "hover:bg-blue-500/10" },
  { icon: BookOpen, label: "Khóa học", href: "/khoa-hoc-ai", color: "text-violet-400", hoverBg: "hover:bg-violet-500/10" },
  { icon: TrendingUp, label: "Flash Sale", href: "/ai-tools?filter=hot", color: "text-orange-400", hoverBg: "hover:bg-orange-500/10" },
]
...
const USER_MENU_SECTIONS = [
  [
    { icon: User, label: "Trang cá nhân", href: "/profile", desc: "Xem & chỉnh sửa thông tin" },
    { icon: Package, label: "Đơn hàng của tôi", href: "/orders", desc: "Theo dõi trạng thái đơn" },
    { icon: Heart, label: "Sản phẩm yêu thích", href: "/wishlist", desc: "Danh sách đã lưu" },
  ],
  [
    { icon: Lock, label: "Bảo mật & Mật khẩu", href: "/profile/security", desc: "Đổi mật khẩu, 2FA" },
    { icon: Settings, label: "Cài đặt tài khoản", href: "/profile/settings", desc: "Ngôn ngữ, thông báo" },
  ],
]
    ...
    <header className="sticky top-0 z-30 w-full border-b border-border/50 bg-background/85 backdrop-blur-xl">
      <div className="max-w-[1600px] mx-auto flex h-16 items-center gap-3 px-4 sm:px-6">
        ...
        <motion.button whileTap={{ scale: 0.9 }} onClick={() => setMobileOpen(true)}
          className="lg:hidden w-9 h-9 rounded-lg flex items-center justify-center hover:bg-accent text-muted-foreground transition-colors" aria-label="Mở menu">
        ...
        <div className="flex-1 max-w-sm">
          <AnimatePresence mode="wait">
            {searchOpen ? (
              <motion.div key="search-active" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative flex items-center">
                <Search className="absolute left-3 w-4 h-4 text-muted-foreground pointer-events-none" />
                <Input autoFocus placeholder="Tìm kiếm AI tools, khóa học..."
                  className="pl-9 pr-10 h-9 rounded-xl border-border/60 bg-card text-sm" onBlur={() => setSearchOpen(false)} />
                <button onClick={() => setSearchOpen(false)} className="absolute right-3 text-muted-foreground hover:text-foreground">
                  <X className="w-3.5 h-3.5" />
                </button>
              </motion.div>
            ) : (
              <motion.button key="search-btn" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                ...
                <kbd className="text-[10px] px-1.5 py-0.5 rounded bg-muted text-muted-foreground font-mono hidden group-hover:inline-block">⌘K</kbd>
              </motion.button>
            )}
          </AnimatePresence>
        </div>
        ...
        <nav className="hidden md:flex items-center gap-1">
          {QUICK_TABS.map(({ icon: Icon, label, href, color, hoverBg }) => {
            const active = pathname === href || pathname.startsWith(href.split("?")[0])
            return (
              <Link key={href} href={href}
                className={cn("relative flex items-center gap-1.5 h-9 px-3 rounded-xl text-sm font-medium transition-all duration-150", hoverBg,
                  active ? `${color} bg-accent` : "text-muted-foreground hover:text-foreground")}>
                <Icon className={cn("w-4 h-4", active && color)} />
                <span className="hidden lg:block">{label}</span>
                {active && <motion.div layoutId="header-active" className="absolute inset-0 rounded-xl bg-accent" transition={{ type: "spring", bounce: 0.2, duration: 0.4 }} />}
              </Link>
            )
          })}
        </nav>
        ...
        <div className="flex items-center gap-1 ml-auto">
          ...
          <div className="relative">
            <motion.button whileTap={{ scale: 0.9 }} onClick={() => setNotifOpen(o => !o)}
              ...
              {notifOpen && (
                <>
                  ...
                  <motion.div initial={{ opacity: 0, y: -8, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.96 }}
                    ...
                    {[
                      { icon: "🎉", title: "Flash Sale đang diễn ra!", desc: "Giảm 40% tất cả AI Tools. Còn 5 giờ!", time: "2 phút" },
                      { icon: "✅", title: "Đơn hàng #VP2024 hoàn thành", desc: "ChatGPT Plus đã được kích hoạt.", time: "1 giờ" },
                      { icon: "📚", title: "Khóa học mới ra mắt", desc: "AI Creative Pro — Đăng ký sớm giảm 30%", time: "3 giờ" },
                    ].map((n, i) => (
                      <div key={i} className="flex gap-3 px-4 py-3 hover:bg-accent/50 transition-colors cursor-pointer border-b border-border/30 last:border-0">
                    ...
                    <div className="px-4 py-2.5 text-center">
                      <button className="text-sm text-blue-400 hover:underline">Xem tất cả thông báo</button>
                    </div>
                  </motion.div>
                </>
              )}
          ...
          <motion.div whileTap={{ scale: 0.9 }}>
            <Link href="/cart" className="relative w-9 h-9 rounded-lg flex items-center justify-center hover:bg-accent text-muted-foreground hover:text-foreground transition-colors" aria-label="Giỏ hàng">
          ...
          {/* Theme */}
          <motion.button whileTap={{ scale: 0.9 }} onClick={() => setTheme(theme === "dark" ? "light" : "dark")}
            className="w-9 h-9 rounded-lg flex items-center justify-center hover:bg-accent text-muted-foreground hover:text-foreground transition-colors" aria-label="Đổi giao diện">
            <AnimatePresence mode="wait">
              {theme === "dark" ? (
          ...
          {/* ── Avatar Dropdown ─────────────────────────── */}
          <div className="relative">
            <motion.button whileTap={{ scale: 0.97 }} id="btn-user-avatar"
              ...
              <div className="hidden sm:block text-left">
                ...
                <p className="text-[10px] text-violet-400 leading-none mt-0.5 font-medium">{user.plan}</p>
              </div>
                ...
                <>
                  ...
                  <motion.div
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="mt-1.5">
                            <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-violet-500/15 text-violet-400 border border-violet-500/20">
                              <Sparkles className="w-2.5 h-2.5" />{user.plan} Plan
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    {USER_MENU_SECTIONS.map((section, si) => (
                      <div key={si} className={cn("py-1.5", si > 0 && "border-t border-border/40")}>
      ...
      <LogoutModal
        ...
        onConfirm={() => { /* TODO: clear auth state, redirect to /login */ }}
        userName={user.name}
